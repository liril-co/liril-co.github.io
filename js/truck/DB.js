export function _user(){
var user= {
    username: "truck",
    users_name: "Juan José Martínez Vidal",
    profilePic: {
            "/truck/img/sdlQg1CoQ3": {
                type: "img",
                date: {
                    stt: "12,12,2021,14,18"
                },
                title: ``,
                description: ``,
                contents: {image: "sdlQg1CoQ3_tn.jpg"},
                Comentarios: ""
            }
        },
    rol:{
       rol: "Dubber",
       level: "88"
    },
     tool: `<div class='tool'>
                <input class='knob button' data-width='102' data-height='102' data-fgColor='#2ecc71' data-bgColor='rgba(0,0,0,0)' data-displayInput=false data-thickness='.08' readonly value='88'>
                <img src='/resources/images/truck/sdlQg1CoQ3_tn.jpg'>
                <div class='datos'>
                    <ul class='actions'>
                        <li class='chatear' title='Chatear'>C</li>
                        <li class='agregar' title='Agregar'>A</li>
                        <li class='juzgar' title='Juzgar'>J</li>
                                     </ul>
                    <p title= 'Juan José Martínez Vidal' class='username'>Juan José Martínez Vidal</p>
                    <p class='rol'>Dubber <b style='color:#fff;'>+88</b></p>
                </div>
                <div class='insignia' style='top: 11px; border-bottom-color: #4f61bf; ' title='Curso completado (Dubbing en español)'></div>
                <div class='insignia' style='top: 23px; border-bottom-color: #79371d; 'title='Curso completado (Sub syncing)'></div>
                <div class='insignia' style='top: 17px; border-bottom-color: #8c8866; ' title='Rol asignado (Dubber)'> </div>
              </div>`,
    cover: {
       insignias: `<div title="<span style= 'color: #4f61bf'>Curso completado (Dubbing en español)</span>" class="line" style="border-bottom: 4px solid #4f61bf; top: 32%;"></div>
                    <div title="<span style= 'color: #79371d'>Curso completado (Sub syncing)</span>" class="line" style="border-bottom: 4px solid #79371d; top: 43%;"></div>
                    <div title="<span style= 'color: #8c8866'>Rol asignado (Dubber)</span>" class="line" style="border-bottom: 4px solid #8c8866; top: 37.5%;"></div>`
    },
    situation: {
    },
    ties: [
    ],
    fotos: {
        quantity: 11,
        fotos: {
            "/truck/img/35CoTKLRMv": {
                type: "img",
                date: {
                    stt: "2,1,2022,1,5"
                },
                title: ``,
                description: ``,
                contents: {image: "35CoTKLRMv_tn.jpg"},
                Comentarios: ""
            },
            "/truck/img/BdsLD8zIGi": {
                type: "img",
                date: {
                    stt: "10,12,2021,14,31"
                },
                title: ``,
                description: ``,
                contents: {image: "BdsLD8zIGi_tn.jpg"},
                Comentarios: ""
            },
            "/truck/img/bHq7JUZnFM": {
                type: "mult_img",
                date: {
                    stt: "27,11,2021,15,28"
                },
                title: ``,
                description: ``,
                contents: {carrContents: ["bHq7JUZnFM_tn.jpg", "HnmFSxVyQi_tn.jpg"]},
                Comentarios: ""
            },
            "/truck/img/XOZD8U9tke": {
                type: "mult_img",
                date: {
                    stt: "24,11,2021,14,53"
                },
                title: ``,
                description: ``,
                contents: {carrContents: ["XOZD8U9tke_tn.jpg", "vEQmEM7Ckv_tn.jpg"]},
                Comentarios: ""
            },
            "/truck/img/sdlQg1CoQ3": {
                type: "mult_img",
                date: {
                    stt: "20,11,2021,15,17"
                },
                title: ``,
                description: ``,
                contents: {carrContents: ["sdlQg1CoQ3_tn.jpg", "xOAtOvdxUN_tn.jpg"]},
                Comentarios: ""
            },
            "/truck/img/fjLURRD0Wy": {
                type: "mult_img",
                date: {
                    stt: "1,11,2021,14,22"
                },
                title: ``,
                description: ``,
                contents: {carrContents: ["fjLURRD0Wy_tn.jpg", "xBfaTRpuKL_tn.jpg"]},
                Comentarios: ""
            },
            "/truck/img/I6xF6sF4Kf": {
                type: "img",
                date: {
                    stt: "15,7,2021,20,21"
                },
                title: ``,
                description: ``,
                contents: {image: "I6xF6sF4Kf_tn.jpg"},
                Comentarios: ""
            }
        }
    },
    videos: {
        quantity: 0,
        videos: {
        }
    },
    stories: {
       "/A.K.A._Dizzy/pos/r1WjYcRQhw": {
            type: "repost_multimedia",
            date: {
                stt: "31,8,2022,15,43"
            },
            originalPoster: {
                username: "A.K.A._Dizzy",
                users_name: "Luis Eduardo Gallego García",
                profilePic: "/resources/images/A.K.A._Dizzy/4uHf7tgk8C_tn.jpg",
                rol: {
                    rol: "CEO",
                    level: "100"
                },
                tool: `<div class='tool'>
                            <input class='knob button' data-width='102' data-height='102' data-fgColor='#2ecc71' data-bgColor='rgba(0,0,0,0)' data-displayInput=false data-thickness='.08' readonly value='100'>
                            <img src='/resources/images/A.K.A._Dizzy/4uHf7tgk8C_tn.jpg'>
                            <div class='datos'>
                                <ul class='actions'>
                                    <li class='chatear' title='Chatear'>C</li>
                                    <li class='agregar' title='Agregar'>A</li>
                                    <li class='juzgar' title='Juzgar'>J</li>
                                                 </ul>
                                <p class='username' title= 'Luis Eduardo Gallego García'>Luis Eduardo Gallego García</p>
                                <p class='rol'>CEO <b style='color:#fff;'>+100</b></p>
                            </div>
                            <div class='insignia' style='top: 39px;' title='EP (Planifique)'> </div>
                        </div>`
            },
            title: "Constructor And Business Partners - Planifique (LQ)",
            description: ``,
            contents: `<media>
                            <div class="media">
                                <audio src="/resources/Un Palmeteo Flamenco.mp3"></audio>
                                <p>Un Palmeteo Flamenco</p>
                            </div>
                            <div class="media">
                                <audio src="/resources/Tal Vez Era Ayer.mp3"></audio>
                                <p>Tal Vez Era Ayer</p>
                            </div>
                            <div class="media">
                                <audio src="/resources/Laura.mp3"></audio>
                                <p>Laura</p>
                            </div>
                            <div class="media">
                                <audio src="/resources/La Olvidaré.mp3"></audio>
                                <p>La Olvidaré</p>
                            </div>
                            <div class="media">
                                <audio src="/resources/Respuestas En Mi Sin Tilde.mp3"></audio>
                                <p>Respuestas En Mi Sin Tilde</p>
                            </div>
                            <div class="media">
                                <audio src="/resources/Punk Für Sie.mp3"></audio>
                                <p>Punk Für Sie</p>
                            </div>
                        </media>`,
            Comentarios: "",//*
        },
        "/L/img/CT8HFtH57Y": {
            type: "repostImg",
            date: {
                stt: "24,7,2022,17,2"
            },
            originalPoster: {
                username: "L",
                users_name: "Laura Escobar Bonnett",
                profilePic: "/resources/images/L/OGnpwD3jys_tn.jpg",
                rol: {
                    rol: "Diseñadora",
                    level: "71"
                },
                 tool: `<div class='tool'>
                            <input class='knob button' data-width='102' data-height='102' data-fgColor='#2ecc71' data-bgColor='rgba(0,0,0,0)' data-displayInput=false data-thickness='.08' readonly value='71'>
                            <img src='/resources/images/L/OGnpwD3jys_tn.jpg'>
                            <div class='datos'>
                                <ul class='actions'>
                                    <li class='chatear' title='Chatear'>C</li>
                                    <li class='agregar' title='Agregar'>A</li>
                                    <li class='juzgar' title='Juzgar'>J</li>
                                                 </ul>
                                <p class='username' title= 'Laura Escobar Bonnett'>Laura Escobar Bonnett</p>
                                <p class='rol'>Diseñadora <b style='color:#fff;'>+71</b></p>
                            </div>
                        </div>`
            },
            title: "...",
            description: ``,
            contents: {image: "CT8HFtH57Y.jpg"},
            Comentarios: `<div class="comentario">
                            <input class='knob button' data-width='28' data-height='28' data-fgColor='#2ecc71' data-bgColor='rgba(0,0,0,0)' data-displayInput=false data-thickness='.18' readonly value='100'>
                                    <img title="
        <div class='tool'>
            <input class='knob button' data-width='102' data-height='102' data-fgColor='#2ecc71' data-bgColor='rgba(0,0,0,0)' data-displayInput=false data-thickness='.08' readonly value='100'>
            <img src='/resources/images/A.K.A._Dizzy/4uHf7tgk8C_tn.jpg'>
            <div class='datos'>
                <ul class='actions'>
                <li class='chatear' title='Chatear'>C</li>
                <li class='agregar' title='Agregar'>A</li>
                <li class='juzgar' title='Juzgar'>J</li>
                                 </ul>
                <p class='username' title= 'Luis Eduardo Gallego García'>Luis Eduardo Gallego García</p>
                <p class='rol'>CEO <b style='color:#fff;'>+100</b></p>
            </div>
            <div class='insignia' style='top: 39px;' title='EP (Planifique)'> </div>
        </div>" src="/resources/images/A.K.A._Dizzy/4uHf7tgk8C_tn.jpg" alt="">
                            &nbsp<a target="_blank" href="/A.K.A._Dizzy">Luis Eduardo Gallego García</a><span class="is">: </span><span class="Comentario">
                                <img src="/resources/images/A.K.A._Dizzy/TmIAHZHTOs.jpg" style="width: 100%; height: auto; border-radius: 0; ">
                            </span>
                            <span class="Responder"></span>
                        </div>`
        },
        "/truck/pos/CMgd0lPQfl": {
            type: "links",
            date: {
                stt: "26,4,2022,21,13"
            },
            title: "Vv11",
            description: ``,
            contents: `<section id="links">
                            <a class="link" target="_blank" href="/truck/p/u889Dj8Q22dSC"><span class="Title">Vv11</span><span class="linktype">Proyecto</span></a>
                        </section>`,
            Comentarios: ""
        },
        "/truck/img/35CoTKLRMv": {
            type: "img",
            date: {
                stt: "2,1,2022,1,5"
            },
            title: ``,
            description: ``,
            contents: {image: "35CoTKLRMv.jpg"},
            Comentarios: ""
        },
        "/truck/img/BdsLD8zIGi": {
            type: "img",
            date: {
                stt: "10,12,2021,14,31"
            },
            title: ``,
            description: ``,
            contents: {image: "BdsLD8zIGi.jpg"},
            Comentarios: ""
        },
        "/truck/img/fjLURRD0Wy": {
            type: "mult_img",
            date: {
                stt: "27,11,2021,15,28"
            },
            title: ``,
            description: ``,
            contents: {carrContents: ["fjLURRD0Wy.jpg", "xBfaTRpuKL.jpg"]},
            Comentarios: ""
        }
    }
}
return user
}